...
WITH updated AS (
    ...
    WHERE token_hash = @token_hash_for_check -- NOTE: token_hashにunique indexがあるため、updated_at, expires_atにインデックスは張らなくてもよい
...
inserted AS (
    INSERT INTO t_jti_blacklist (jti, expires_at) SELECT access_token_jti, @expires_at FROM deleted ON CONFLICT DO NOTHING RETURNING jti
)
